...
//! This links against the system libdisplay-info to validate correctness and compare performance.
...
fn generateDellTestEdid(allocator: std.mem.Allocator, log: *cli.Logger) ![]u8 {
...
/// Public entry point to run libdisplay-info comparison benchmark
pub fn run(allocator: std.mem.Allocator, log: *cli.Logger) !void {
    ...
    log.info("", .{});
    log.info("╔" ++ "═" ** 68 ++ "╗", .{});
    log.info("║" ++ " " ** 13 ++ "Zig vs C libdisplay-info Benchmark" ++ " " ** 21 ++ "║", .{});
    log.info("╚" ++ "═" ** 68 ++ "╝", .{});
    log.info("", .{});
